...
@section('content')
    ...
    @if (session('status'))
    <div class="notification is-success" role="notification">
        {{ session('status') }}
    </div>
    ...
    <div class="column is-one-third is-offset-one-third m-t-50">
        <div class="card">
            ...
            <form action="{{ route('password.request') }}" method="POST" role="form">
                ...
                <p class="control">
                    <input class="input {{ $errors->has('name') ? 'is-danger' : '' }}" type="name" id="name" placeholder="Name" name="name" value="{{ old('name') }}">
                </p>
                @if($errors->has('email'))
            ...
            <h5 class="has-text-left m-t-30 m-l-5"><a href="{{ route('login') }}" class="is-muted">Already Have An Account?</a></h5>
            </div>
